...
表示全时域。Inactive node 及其 subtree 不参与 attachment、clip、layout 或绘制。Visibility false
同样使该 node 及其 subtree 不参与绘制，但不改变 sibling；这使 Group/ClipGroup 的 visibility
只有一种解释。Opacity 0 仍是存在的透明绘制，可能影响 `copy` composite。
...
clockwise/counterClockwise，并按 FCS Y-up 坐标解释；除零 sweep 外，clockwise 要求
`endAngle-startAngle<0`，counterClockwise 要求 `>0`。Runtime 可以改变 command 的数值参数，
不能改变 command 数量和类型。
...
cycle、越界或超限使用 `render.resource-decode-failed`。
...
或 strokeRef。任何 node 的 clipRef 都在自身 draw和 subtree 上生效。
...
Arc/EllipseArc 的 angle difference 使用保存的有符号差并必须与 direction 满足第 7 章；start=end 是
零 arc，绝对差大于一整 turn 时保留全部 turns，不按 modulo丢失。
...
所有 metric finite。Font resource 必须满足第 11 章；faceIndex 在 Core profile 必须 0，glyphId 小于
font numGlyphs。Glyph count 可以为 0，仅用于空 source content。RenderSection minor 0 的
...
`render.invalid-descriptor`。`p` 只在 descriptor 自身 Segment求值时可用，不形成外部隐式 context。
...
list，不读取 source fixture。它先在同一 query time求 attachment 与所有 exact descriptor，再按第 5
章递归 flatten。Inactive/visibility-false parent移除 subtree；每个 local/attachment/parent/world 3×3
matrix、effective opacity、clip chain 和 evaluated property 都使用 `fcs.md` binary64逐操作规则。
...
stroke 保持 open；Polygon fill/stroke 都 closed。Path 的 quadratic/cubic/arc/ellipse arc 语义是精确
...
segment参与 dash。Sample 恰在 stroke boundary 上算 inside。
...
| 3 | Singleton/count/table boundary/known payload 未恰好消费 | `render.invalid-section` |
| 4 | Render nested Record byteLength/version/flags/tail、Value framing/tag/padding | `render.invalid-record` |
| 5 | Duplicate/zero ID、table canonical order、Layer root range、parent order/cycle、orphan ownership | `render.invalid-graph` |
| 6 | Render u32 table ref越界、nullability或 kind incompatibility（Geometry schema除外） | `render.invalid-reference` |
| 7 | Geometry object key/tag/count、negative/invalid geometry、Path command状态 | `render.invalid-geometry` |
| 8 | Render direct-root index/type/domain/environment错误 | `render.invalid-descriptor`；descriptor kind 5 仍是 `fcbc.forbidden-descriptor` |
| 9 | Render resource ID 不存在 | `render.resource-not-found` |
| 10 | Resource kind 或已声明 media type与 owner不符 | `render.resource-type-mismatch` |
| 11 | 已知 kind/media但 Core codec/shaping/profile feature不支持 | `render.resource-capability-missing` |
| 12 | 已支持 codec/font profile 的 bytes/metadata/table/outline malformed | `render.resource-decode-failed` |
| 13 | decode/evaluate 后 descriptor value非有限、类型或值域非法 | `render.invalid-descriptor` 或 `render.invalid-geometry`，按 owner field |

任何 limit 在对应步骤读取 file-controlled count、dimension、table 或分配前检查，并覆盖该步骤的
其他语义错误，返回 `render.limit-exceeded`；更早的 FCBC structural/checksum/hash 仍优先。一个
...
和 GlyphRun source-text tail；另有不修 CRC 的 checksum case。Fixture asset 必须有项目内 provenance/
license，测试不得读取系统字体、外部 URL 或 workspace fallback。
