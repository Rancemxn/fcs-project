...
| 规范 | 候选版本 | 状态 |
|---|---:|---|
| FCS Core Source Specification | 5.0.0 | Draft（I1 source-parser dependency closure 已建立阶段 baseline 并开始实现；canonical fixture、其余 blocker 与最终联合复审仍开放） |
| FCBC Container Format | 2.0.0 | Draft（one-chart/ResourceData/exact-only 与历史非空 ABI artifact 已独立复审；当前 visibility owner-environment delta 路由 I7，另等待 Render byte vector、其余 blocker 与最终联合复审） |
| FCS Execution ABI | 1.0.0 | Draft（非空 byte/evaluation/mutation 历史 blocker 已独立复审关闭；当前 `note.presentation.visibility` EnvB golden 例外须在 I7 重建复审，另等待其余跨规范 blocker 与最终联合复审） |
| FCS Render Profile | 1.0.0 | Draft（失败快照后的 RNR 候选修订已在新 hash 上独立复审通过；完整 executable artifact、其余 blocker 与最终联合复审仍开放） |
