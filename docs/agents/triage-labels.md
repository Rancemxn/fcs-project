...
relationship. Create a milestone when a real stage or coordinated workstream needs one; do not require a
milestone or due date for every Issue.

Contributors may use existing milestones and labels within their authorized work. Changes to the shared taxonomy
require maintainer agreement; they do not depend on a named agent session or pool role.
...
When using the GitHub CLI, update the old and new state labels in one operation:
...
- relevant Accepted ADRs, conformance artifacts, reviews, and stage baseline when the work affects them;
