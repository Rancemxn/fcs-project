...
状态：Draft（2026-07-22；I10 executable closure remains open；见 `docs/specifications/governance.md`）
...
`scrollSpeed` 是相对于本 Line 的 q 的无量纲 multiplier，可以依赖 `s/b/q` 和常量，但不能依赖
Note `d`。先定义 Line-local 瞬时速度和累计距离：

```text
localVelocity_i(t) = scrollSpeed_i(t) * scrollBpm_i(t) / 60

localFloor_i(t) = initialFloorPosition_i
                + integral[integrationOrigin_i,t](scrollSpeed_i(u) dq_i(u))
```
...
默认 scrollSpeed 为 `1.0`。负值只有声明该 local speed 的 Line 显式设置
`allowReverseScroll: true` 时合法；零值停止该 Line 的 local scroll，但不停止 chartTime。Speed
可以在边界跳变，`localFloor` 必须连续。父 Line 是否允许 reverse 不改变子 Line local speed 的
合法性；继承后的有效速度可以因为一个显式允许 reverse 的祖先而为负。

### 10.3 Floor 和 Note Y

`localFloor` 是每条 Line 自己的无量纲累计距离。用于 Line/Note 查询的 `effectiveFloor` 按第 11.4
节沿实际 parent ancestry 组合；`effectiveVelocity` 使用同一 ancestry 的 local velocity。`q_i` 始终
保持 Line-local，不因 scroll inherit 改成 parent q 或第二物理时钟。

`floorScale` 是每个 effective floor unit 对应的 logical length。Note 使用所属 Line 的
`effectiveFloor`：

```text
localY(note,t) =
    (effectiveFloor(note.line,note.gameplay.time)-effectiveFloor(note.line,t))
    * floorScale
...
每个 Line-local Distance 必须选择 `portable-analytic` 或 `portable-evaluable`。Scroll inheritance
不创建第三种 classification、第二份 absolute-floor descriptor 或 BakedCurve；它在查询时按第 11.4
节组合已有 local Distance。标准 FCS→CanonicalChart→FCBC 路径不能因为
...
组合可以使最终矩阵包含几何 shear，但 source 仍未声明独立 shear property。

Scroll 使用独立的递推，不进入几何 component state：

```text
effectiveVelocity_i(t) = localVelocity_i(t)
                       + (inherit.scroll && parent
                          ? effectiveVelocity_parent(t)
                          : 0)

effectiveFloor_i(t) = localFloor_i(t)
                    + (inherit.scroll && parent
                       ? effectiveFloor_parent(t)
                       : 0)
```

`inherit.scroll=false` 在当前 Line 终止 scroll ancestry；`true` 只访问实际 parent chain，不访问 sibling、
声明顺序更早的 Line 或其他无关 Line。每个 local descriptor 保留自己的 q、scroll tempo、speed、
integration origin、initial floor position 和 boundary。Parent/child 的 q 不相加，floorScale、Note
scrollFactor、Line transform 和 textureAnchor 也不进入上述递推。

`effectiveVelocity` 从 root 到目标 Line 按该顺序逐项执行 binary64 addition，任何非有限中间值产生
structured execution error。`effectiveFloor` 的 reference 值在高精度域中把 ancestry 上各
`localFloor` 的 reference 值相加，最后只舍入一次为 binary64；若没有 parent contribution，则必须
保留 local Distance 的 origin/signed-zero 规则。继承和 parent 查询发生错误时只沿实际 ancestry
传播；无关 Line 的 gap、domain、overflow 或其他失败不得改变本 Line 查询。

Direct seek 每次从各 local integration origin 直接查询并组合，不依赖之前的 frame、缓存内容或
顺序播放历史。一个祖先启用 reverse 可以使后代 `effectiveVelocity` 为负，即使后代自己的
`allowReverseScroll` 为 false；该 flag 仍只验证声明在该 Line 上的 local speed。
...
lineScrollCoordinate(lineId, chartTime)  # Line-local q
lineScrollDistance(lineId, chartTime)    # inherited effectiveFloor
...
deterministic。Line scroll distance query 只允许访问目标 Line 的 scroll parent ancestry；无关 Line
的 descriptor 错误不得抢先失败。
