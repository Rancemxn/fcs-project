...
## I6.6 Repair execution boundary

I6.6 增加 `apply_authorized_repair` 与 repair/profile-selection 不变量：仅授权 rule 可写入
`RepairRecord`；`repair_enabled` 不得把合法 profile 歧义变为 chosen binding。

## I6.7 Public fixtures and opt-in copyright lane

I6.7 增加可执行公开 fixture lane：`docs/conformance/conversion/public-fixtures/` 持有
minimal/feature/extreme 合成源、expected ConversionReport/canonical shape keys，以及
`fcs_conversion::fixture_lane` 产品 harness。每个 fixture 声明 dialect/profile/evidence/policy，
经真实 parse→interpret→lower 比较 status、line/note counts、required categories 与 provenance keys。
私有版权 lane 仅通过 `FCS_COPYRIGHT_FIXTURE_ROOT` opt-in；默认/CI 报告 `skipped`，不随仓库分发
版权谱面。

## 后续单元

I6 只有在上述单元共同交付真实 source-to-canonical execution 与 round-trip 证据后才完成；I6.1 只是
共享的 lossless trust boundary。I8 负责 exporter reparse/round-trip。
