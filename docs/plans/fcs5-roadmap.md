...
I4.4 规范闭包 Issue #223 进一步固定 Line-local/effective scroll composition、parent ancestry、
direct seek、reverse/error isolation 和 literal `source.valid.scroll-inheritance` vector；这仍是
candidate specification/fixture evidence，不声称 evaluator product 或 I4 完成。I4.4–I4.8 的 scroll/DAG/
Piecewise/integration/reference cross-check 仍是后续 residual，任何 sampled/BakedCurve 路径都不进入标准 runtime。
