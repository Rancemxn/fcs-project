...
当前交付状态：I3.1–I3.5 已实现并合并；I3.5 已建立 immutable canonical Note、四种 Note kind、
Line/time/policy/shape/Hold/presentation lowering、stable ID 和 deterministic sort 的实现与 focused
evidence。I3.5 的阶段 residual 是 Track normalization、scroll integration、runtime visibility/descriptor、
resource bytes/hash resolution、CanonicalChart aggregation 和 snapshot；I3.6–I3.9、I4 和 I5 继续承接，
这些 residual 不阻塞已合并的 Note work-unit。
...
- I3.1–I3.5 已实现并合并；I3.3 metadata/resource/sync、I3.4 Line graph/topology 及其 span corrective
  baseline、I3.5 Note lowering 均已交付，当前 I3.6 Track normalization 是下一 bounded frontier。
  I3.6 的计划与边界记录在 `docs/plans/i3-track-normalization.md`；其 full-gate/merged-SHA 证据仍待
  本 work unit 交付。I3.6 之后的 residual 为 scroll、runtime descriptor、CanonicalChart/snapshot 和
  I5 resource bytes/hash resolution；I3.7–I10 尚未完成。
