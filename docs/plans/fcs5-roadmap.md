...
当前交付状态：I3.1–I3.6 已实现并合并；I3.5 已建立 immutable canonical Note、四种 Note kind、
Line/time/policy/shape/Hold/presentation lowering、stable ID 和 deterministic sort 的实现与 focused
evidence，I3.6 已建立 immutable Track normalization。当前 bounded frontier 是 I3.7 scroll model；
其后 residual 是 runtime visibility/descriptor、resource bytes/hash resolution、CanonicalChart
aggregation 和 snapshot，由 I3.8–I3.9、I4 和 I5 继续承接。
...
- I3.1–I3.6 已实现并合并；I3.3 metadata/resource/sync、I3.4 Line graph/topology 及其 span corrective
  baseline、I3.5 Note lowering 和 I3.6 Track normalization 均已交付。当前 I3.7 scroll model 是下一
  bounded frontier，其计划与边界记录在 `docs/plans/i3-scroll-model.md`。I3.7 之后的 residual 为
  runtime descriptor、CanonicalChart/snapshot 和 I5 resource bytes/hash resolution；I3.8–I10 尚未完成。
