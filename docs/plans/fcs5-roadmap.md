...
当前交付状态：I3.1–I3.7 已实现并合并；I3.7 已建立 exact canonical scroll coordinate、chart-time
normalization、constant-speed floor position 与 focused evidence。I3.8 implementation branch 已建立
immutable `CanonicalChart` aggregation 和单一 source compiler seam，交付仍受 full gate 与 Issue/PR
workflow 约束。其后 residual 是 runtime visibility/descriptor、resource bytes/hash resolution 和
canonical snapshot，由 I3.9、I4 和 I5 继续承接。
