...
当前交付状态：I3.1–I3.9 已实现并合并；I3.8 已建立 immutable `CanonicalChart` aggregation 和单一
source compiler seam，I3.9 增加测试专用的显式 `serde_json::Value` projection、pretty snapshot、
direct/template byte equality 与单一 golden，并验证 snapshot 不保留 source/workspace/authoring/
preserve 状态。I3.9 不建立产品或规范性 JSON 格式。其后 residual 是 I4 runtime visibility/descriptor
与 I5 resource bytes/hash resolution。
...
- **I4.4 Scroll evaluator**：q、Line-local velocity/floor、parent `inherit.scroll` composition、
  analytic/piecewise integral、distance continuity、negative/zero speed、signed zero 和 direct seek；
  effective floor 在 ancestry 上高精度合并后只舍入一次，FCBC 保持 Line-local Distance wire layout。
...
I4.3 在 immutable canonical Line/Track boundary 上增加 project-owned transform query、pivoted column-vector
matrix、component-wise parent inherit、world component/origin 递推与稳定错误；production 私有使用 cataloged
`nalgebra` 0.35.0 fixed-size storage，但 3×3 运算顺序由项目代码显式固定，且不作为 I4.8 independent reference。
I4.4 规范闭包 Issue #223 固定了 Line-local/effective scroll composition、parent ancestry、direct
seek、reverse/error isolation 和 literal `source.valid.scroll-inheritance` vector；Issue #225 现已将
`fcs_runtime::{evaluate_line_scroll,evaluate_note_distance}` 接到 canonical Line/Scroll/Track/Note
边界，并由该 fixture 执行产品查询。精确 head 的 full gate、Primary Self-Audit 和独立复审仍是该
work-unit 的交付证据；Issue #227 现已将直接 runtime field 接到 source-free typed Expression DAG，
...
family/branch 向量、独立 Bezier solve、矩阵/scroll cross-check 和严格可执行 numeric corpus。I4.9
已激活 dev-only `proptest` 1.11.0，以固定 ChaCha seed、96 cases 和最多 16 个 frame partition 覆盖
constant/step/linear/Core easing/Bezier、正反向 direct-seek、partition/frame-rate invariance、独立
linear analytic error bound、raw-bit repeatability、Track/Line declaration order、randomized transform
inherit 以及 stable reverse/non-finite error。I4.1–I4.9 的计划实现单元已闭合；这不改变任何规范
version-domain 状态，也不宣称 FCS 5 完成。FCBC descriptor assembly/serialization 仍归 I7，任何
...
- The I10 `check` command now reuses `canonical_compilation_with_source`, so a successful check covers
  ...
  solid-fill and stroked Path descriptor evaluation, curve flattening, fill-rule containment, explicit Close/open-subpath
  stroke behavior, dash/cap/join handling, and reference-raster coverage; source Line stroke lowering, source
  ImagePattern lowering, source Path lowering, source Stroke grammar, Path text coverage, broader canonical writer
  coverage, Polyline/Text geometry, broader resource semantics, isolated compositing, and semantic/raster conformance
  remain open under #296/#9.
