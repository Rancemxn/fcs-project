...
Proptest robustness 和强类型 manifest 完整性门。当前 workspace 有 175 个通过的测试，其中 9 个
属于 test-only Execution ABI lane、5 个属于 test-only RenderSection lane；这只证明 I0 retained subset、
...
| `fcs.md` 2.4 | ASCII identifier、keyword 与 field-name context | `parser::parse_expression`, `parser::parse_document` | `parser/token.rs`, `parser/lexer.rs` | `lexer::every_core_reserved_word_has_one_keyword_token`, `lexer::contextual_render_words_and_keyword_fields_keep_core_token_kinds` | `diagnostic::additional_bom_and_non_ascii_identifier_spans_are_exact`, `expression::token_parser_rejects_reserved_names_and_trailing_input`, `source.invalid.unresolved-schema-enum`（manifest only） | partial | I1 | 完整 Core keyword 集、ASCII identifier 与 keyword field token context 已闭合；完整 field-path/source AST 和 unresolved enum 保留由 Task 2/6 完成 |
...
| `fcs.md` 2.8–2.10 | 分隔符、array/object/reference、半开 interval、closed enum | `parser::parse_expression` | `parser/lexer.rs`, 未来扩展 `parser/expression.rs`, `ast/types.rs` | `lexer::punctuation_and_longest_match_tokens_are_complete`, `source.valid.track-boundaries`, `source.valid.complete-source-grammar`（后两项 manifest only） | `lexer::unsupported_standalone_punctuation_is_rejected`, `source.invalid.unresolved-schema-enum`（manifest only） | partial | I1 | 完整 delimiter/operator token 与 bare `..` 拒绝已闭合；array/object/reference、唯一半开 interval、schema-only cubic Bezier AST、string enum 和 keyword field parser 由 Task 2/6 完成 |
...
| `fcs.md` 8.1–8.3 | chartTime、tempo、offset、judgment/scroll 与 import-time source clocks | `ast::TempoMap` | `parser/tempo.rs`, `ast/time.rs`, `validation.rs`, 未来 `fcs-model` | `frontend::parses_tempo_map_after_line_comment`, `lexer::every_literal_and_unit_suffix_has_one_token` | `frontend::i0_retained_tempo_parser_rejects_bad_mixed_beat_and_bpm`, `frontend::rejects_removed_mixed_beat_literal`, `frontend::tempo_points_must_be_non_decreasing`, `source.invalid.mixed-beat-literal`（manifest only） | partial | I1/I3/I6 | mixed Beat 已删除，signed/zero source BPM 由未验证的 `SourceBpm` 表示后再报 `tempo.invalid`；现有 zero-start/monotonic/profile validation 仍在 Parse API 中，须由后续 I1 document/fixture tasks 恢复 AST-only phase boundary，I3 再规范化 chartTime，I6 按 profile 解码外部时钟 |
