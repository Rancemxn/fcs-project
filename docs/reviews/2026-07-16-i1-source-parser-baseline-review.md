...
- `docs/conformance/fcs5/manifest.toml` 的全部 source input、stage、expect、diagnostic 和 clauses binding；
...
| `docs/conformance/manifest.toml` | `231F4505DE29C854201057F97706295756109A98DCC7AC99F08CA21CD3F96FE8` | root suite binding |
| `docs/conformance/fcs5/manifest.toml` | `4D8DFB7BA2D636CD94A02F39807C7A0DA6185B73213F50DD77E8F2A69C5B25F4` | 39-entry FCS source manifest |
...
Fixture tree 固定为 `docs/conformance/fcs5` 下的 `manifest.toml` 加全部 38 个 `**/*.fcs`，共 39 files。
路径必须相对 `docs/conformance/fcs5`、按 ordinal byte order 排序，并把每个 Windows separator 规范化为
...
| full cross-spec executable docs/conformance/Frozen | I10 | 是发布门，不是 I1 implementation input |
