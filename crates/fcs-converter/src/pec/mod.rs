...
pub fn parse_pec(text: &str) -> Result<IrChart, String> {
        ...
        match tok[0].as_str() {
            "bp" if tok.len() >= 3 => {
                ...
                bpm_list.push(IrBpmPoint {
                    beat: tok[1].parse::<f64>().unwrap_or(0.0),
                    bpm: tok[2].parse::<f64>().unwrap_or(120.0),
                });
            }
            s if s.starts_with('n') && s.len() == 2 && tok.len() >= 5 => {
    ...
    let all_notes: Vec<IrNote> = notes_raw
        .iter()
        .enumerate()
        .map(|(i, raw)| {
            ...
            let hold_time = if kind == IrNoteKind::Hold && raw.len() >= 6 {
                raw[5].parse::<f64>().unwrap_or(0.0)
            } else {
                0.0
            };
            ...
            IrNote {
                kind,
                above: raw[1].parse::<i32>().unwrap_or(1) == 1,
    ...
    let line = IrLine {
        name: "line0".into(),
        notes_above: all_notes.iter().filter(|n| n.above).cloned().collect(),
        notes_below: all_notes.iter().filter(|n| !n.above).cloned().collect(),
        events: IrEventBundle::default(),
        bpm: default_bpm,
        z_order: 0,
        texture: None,
    };
