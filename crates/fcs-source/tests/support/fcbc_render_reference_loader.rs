...
use super::fcbc_reference_loader::{
    self, DecodedChart, DescriptorKind, ExpressionNode, PropertyDescriptor, ValueType,
};
use super::fcbc_render_reference_assets::{
    AssetError, DecodedImage, TestFont, decode_font, decode_image,
};
...
impl NodeRecord {
    ...
    fn active_domain(&self) -> fcbc_reference_loader::Domain {
        fcbc_reference_loader::Domain {
...
pub fn load_render(bytes: &[u8]) -> Result<DecodedRenderChart, &'static str> {
    let core = fcbc_reference_loader::load(bytes)?;
...
fn check_descriptor(
    ...
    fcbc_reference_loader::validate_descriptor_env_p_context(
        reference,
