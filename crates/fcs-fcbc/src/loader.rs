...
///
/// Failures return the nearest stable `fcbc.*` parent diagnostic. Framing-only
/// validation without Core section decode is available via
/// [`crate::load_container`].
pub fn load_chart(bytes: &[u8]) -> Result<DecodedChart, &'static str> {
    load(bytes)
}
...
fn parse_container(bytes: &[u8]) -> Result<ParsedContainer, &'static str> {
    ...
    // FCBC §3.1: 0=runtime, 1=fidelity, 2=reserved (reject), 3=strict-runtime.
...
fn parse_constant_pool(
    ...
    for index in 0..count {
        ...
        let value = parse_runtime_constant(&mut cursor).map_err(|error| {
            // Non-finite control payloads must use the Expression diagnostic,
            // even though the Value decoder rejects them before type checking.
    ...
    for (index, node) in nodes.iter().enumerate() {
        ...
        if node.opcode == 64 {
            for operand in &node.operands[1..] {
                ...
                // With fixed endpoints 0 and 1, x1,x2 in [0,1] make x(t)
                // strictly increasing, including x1=1,x2=0's isolated flat point.
...
/// Depth-checked entry for the expression dependency walk, kept for the
/// validator tests: the production walks enter subgraphs at per-occurrence
/// depths through `expression_dependency_facts` directly.
#[cfg(test)]
fn expression_environment_dependencies(
    ...
    let mut cache = vec![None; expressions.len()];
    let (dependencies, longest) = expression_dependency_facts(index, expressions, &mut cache)?;
    if depth + longest > MAX_VALIDATOR_DEPTH {
        return Err("fcbc.limit-exceeded");
    }
    if depth + longest > expressions.len() {
        return Err("fcbc.invalid-expression");
    }
    Ok(dependencies)
}
