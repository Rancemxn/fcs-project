...
pub fn evaluate(easing_id: u8, t: f64) -> f64 {
    ...
    match easing_id {
        ...
        20 => {
            let c = 1.70158;
            let c3 = c + 1.0;
            1.0 + c3 * (t - 1.0).powi(3) + c * (t - 1.0).powi(2)
        }
        21 => {
            let c = 1.70158;
            let c3 = c + 1.0;
            c3 * t.powi(3) - c * t.powi(2)
        }
        22 => {
            if t < 0.5 {
        ...
        24 => {
            ...
            2.0f64.powf(-10.0 * t) * ((10.0 * t - 0.75) * 2.0 * std::f64::consts::PI / 3.0).sin()
                + 1.0
        }
        25 => {
            if t <= 0.0 {
            ...
            -2.0f64.powf(10.0 * t - 10.0)
                * ((10.0 * t - 10.75) * 2.0 * std::f64::consts::PI / 3.0).sin()
        }
...
fn bezier_x_deriv(t: f64, cx1: f64, cx2: f64) -> f64 {
    3.0 * (1.0 - t).powi(2) * cx1
        + 6.0 * (1.0 - t) * t * (cx2 - cx1)
        + 3.0 * t.powi(2) * (1.0 - cx2)
}
