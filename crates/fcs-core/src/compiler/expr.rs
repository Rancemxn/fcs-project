    ...
    match expr {
        ...
        Expression::Variable(name) => {
            code.push(match name.as_str() {
                "s" => Opcode::PushVarS, "b" => Opcode::PushVarB,
                "d" => Opcode::PushVarD, _ => Opcode::PushVarB,
            } as u8);
        }
        Expression::BinaryOp { op, left, right } => {
            ...
            code.push(match op {
                ...
                BinaryOp::Mod => Opcode::Mod, BinaryOp::Pow => Opcode::Pow,
            } as u8);
        }
        ...
        Expression::Ternary { cond, if_true, if_false } => {
            ...
            code.push(Opcode::JmpIfFalse as u8); code.extend_from_slice(&[0u8; 2]);
            compile_expr(if_true, consts, code);
            let jmp_end_pos = code.len();
            code.push(Opcode::Jmp as u8); code.extend_from_slice(&[0u8; 2]);
            let ft = code.len();
        ...
        Expression::ChainCompare { left, ops } => {
            ...
            for (cmp, right) in ops {
                if !first { code.push(Opcode::Swap as u8); }
                compile_expr(&prev, consts, code);
                compile_expr(right, consts, code);
                code.push(match cmp {
                    CompareOp::Lt=>Opcode::CmpLt, CompareOp::Le=>Opcode::CmpLe,
                    CompareOp::Gt=>Opcode::CmpGt, CompareOp::Ge=>Opcode::CmpGe,
                    CompareOp::Eq=>Opcode::CmpEq, CompareOp::Ne=>Opcode::CmpNe,
                } as u8);
                if !first { code.push(Opcode::Mul as u8); }
...
fn compile_call(name: &str, args: &[Expression], consts: &mut ConstantPoolBuilder, code: &mut Vec<u8>) {
    if let Some(op) = builtin_opcode(name) {
        for a in args { compile_expr(a, consts, code); }
        code.push(op as u8);
...
fn builtin_opcode(name: &str) -> Option<Opcode> {
    Some(match name {
        ...
        "atan2"=>Opcode::Atan2,"abs"=>Opcode::Abs,
        "exp"=>Opcode::Exp,"ln"=>Opcode::Ln,"log2"=>Opcode::Log2,"log10"=>Opcode::Log10,
        "sqrt"=>Opcode::Sqrt,"floor"=>Opcode::Floor,"ceil"=>Opcode::Ceil,"round"=>Opcode::Round,
        "clamp"=>Opcode::Clamp,"lerp"=>Opcode::Lerp,"min"=>Opcode::Min,"max"=>Opcode::Max,
        _=>return None,
    })
}

fn is_easing(s: &str) -> bool { s.starts_with("ease") }

fn easing_id(name: &str) -> u8 { match name {
    "easeLinear"=>1,"easeOutSine"=>2,"easeInSine"=>3,"easeOutQuad"=>4,"easeInQuad"=>5,
    "easeInOutSine"=>6,"easeInOutQuad"=>7,"easeOutCubic"=>8,"easeInCubic"=>9,
    "easeOutQuart"=>10,"easeInQuart"=>11,"easeInOutCubic"=>12,"easeInOutQuart"=>13,
    "easeOutQuint"=>14,"easeInQuint"=>15,"easeOutExpo"=>16,"easeInExpo"=>17,
    "easeOutCirc"=>18,"easeInCirc"=>19,"easeOutBack"=>20,"easeInBack"=>21,
    "easeInOutCirc"=>22,"easeInOutBack"=>23,"easeOutElastic"=>24,"easeInElastic"=>25,
    "easeOutBounce"=>26,"easeInBounce"=>27,"easeInOutBounce"=>28,"easeInOutElastic"=>29,
    "easeBezier"=>30, _=>0,
}}
