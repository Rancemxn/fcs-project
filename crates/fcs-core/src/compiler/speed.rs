...
pub fn classify_speed(expr: &Expression, consts: &mut ConstantPoolBuilder) -> SpeedTier {
    ...
    if depends_runtime {
        SpeedTier::Tier3 { velocity_bc }
    } else {
        SpeedTier::Tier2
    } // MVP: always Tier2 for non-const
}
...
fn expr_depends_on_runtime(expr: &Expression) -> bool {
    match expr {
        ...
        Expression::BinaryOp { left, right, .. } => {
            expr_depends_on_runtime(left) || expr_depends_on_runtime(right)
        }
        ...
        Expression::Ternary {
            ...
            expr_depends_on_runtime(cond)
                || expr_depends_on_runtime(if_true)
                || expr_depends_on_runtime(if_false)
        }
