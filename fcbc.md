...
逐个前进，并跳过 `recordVersion=1` record 末尾位于 record boundary 内的未知扩展 bytes；Writer
2.0.0 不生成扩展尾部，任何为 4-byte 对齐加入的尾部 byte 都写零。Record 的内部 `flags` 字段
...
`render.geometry.radiiDescriptor[2]`），不是 source spelling、局部 Track ID 或 workspace path。
