...
状态：Draft（2026-07-16；非空 Execution ABI artifact 已独立复审，等待 Render/Conversion/Core
blocker 与最终联合独立复审）
...
外层一致，其 domain 必须覆盖该 Piece interval；求值使用同一个完整 environment，只由 Piece 选择
被引用 descriptor，不修改 EnvS/EnvB/EnvQ/EnvD/EnvP。依赖图必须无环。
...
`fcbc.invalid-expression`/`fcbc.invalid-track` 分别拒绝。一个 Expression descriptor 的 environment
availability 取第 13 章所有 direct/transitive owner root 对应 target schema 的交集；该集合不会为空。
...
unused operand、immediate、environment availability 和整个 DAG。Unknown opcode 使用
`fcbc.invalid-expression`。每个 PropertyDescriptor root 的 resultType 必须与其 propertyType 相同；
node-only vector type 只能出现在 root 之前。Env dependency 必须满足所有引用该 descriptor 的
target schema；共享 descriptor 使用这些 schema 允许环境的交集。Runtime 对 invalid math 返回
...
逐 bit/误差验证由 conformance vector 执行。
...
这两个 Line-owned descriptor 是唯一规范真相；Distance 不保存第二份 integrand DAG 或可与它们
矛盾的 absolute-floor function。
...
两个 root 的 reachable SegmentTrack point/start/end 与 Piecewise bounded start/end；它们依赖的
global TempoMap/EnvB point time；以及 scrollTempo 导出的 q 分段边界。除此之外不得加入“有助于某
积分器”的任意点或统一采样格；Expression 内的连续 predicate threshold 不因 compiler 猜测而成为
...
这里的 reference floor 是 `fcs.md` 第 10.2 节连续积分公式：对每个实数积分变量 `u`，先把
...
12. Render singleton/Record/table/ID/order/reachability、Render direct-root type/domain/environment 与
    resource kind/media/capability binding；Render 内部 precedence 由 `fcs-render.md` 第 16 章固定；
...
14. HAS_RENDER、viewport、八张 Render table、image/WebP/font resource、semantic draw-list、reference
    raster、exact descriptor root、no-source-text GlyphRun 与 `render.*` mutation precedence。
